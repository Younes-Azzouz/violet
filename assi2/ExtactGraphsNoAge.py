...
class Rabbit(Agent):
    ...
    def update(self):
        ...
        age = """12 #time.time() - self.birth_time
        ...
        if self.reproduction_timer > 240:  # Check the timer
            ...
            for agent, _ in potential_mates:
                if agent.alive() and agent.gender != self.gender:
                    if random.random() < 0.02 and self.energy_bar > 30:  # Chance to reproduce when encountering an opposite gender rabbit
                        self.reproduce() 
...
class Fox(Agent):
    ...
    def update(self):
        ...
        age = """2 #time.time() - self.birth_time
        ...
        if self.reproduction_timer > 240:  
            ...
            for agent, _ in potential_mates:
                if agent.alive() and agent.gender != self.gender:
                    if random.random() < 0.02 and self.energy_bar > 30:  # Chance to reproduce when encountering an opposite gender fox
                        self.reproduce()  
                        self.reproduction_timer = 0  # Reset the timer after reproduction
...
for i in range(20):
    ...
    plot.savefig(f'assi2/Graphs/Seed{i}_pop_2min_no-age.png', dpi=600)
    df.write_csv(f'assi2/CSV Graphs/Seed{i}_pop_2min_no-age.csv')
